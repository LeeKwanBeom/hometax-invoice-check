...
> **4차 추가 (2026-09-06, 실사용 실행):** 신규 결함 2건 — #26 `해결` 집계
> 누락(높음), #27 `--force` 예외 절차 부재(중간). 아래 "신규 결함 (4차)" 절 참조.
> 이번 회차는 코드를 고치지 않았다. vendors.json 에 3곳 추가(플리드 FLID·
> 인투 란 컴퍼니 `비정기`, 팩프렌즈 `until=2026-04`) 하고 실행 이력을
> 사용자 승인 하에 `--force` 로 반영했다.
...
## 신규 결함 (2026-09-06 4차 — 실사용 실행에서 발견)

앞선 1~3차는 픽스처로 돌린 점검이었다. 이 두 건은 **실제 홈택스 자료로
평소처럼 실행하다가** 드러났다. 둘 다 산출물 숫자는 맞는데 **사람에게
전달되는 요약이 틀리게 만드는** 종류다.

| # | 심각도 | 파일 | 줄 | 원문 | 왜 틀렸는지 | 수정 방향 |
|---|---|---|---|---|---|---|
| 26 | **높음** | `scripts/build_report.py` · `scripts/validate.py` | 568, 576 / 419-420, 430 | `n = {g: sum(1 for r in rows if r["grade"] == g) for g in list(GRADE_ORDER) + ["해결"]}` / `elif g == "해결": resolved.add(sid)` | **`해결` 집계가 `등급` 열만 세는데, 실제 해결 건은 `상태` 열에 적힌다.** #24 수정으로 들어온 `by_id[bid]["status"] = "해결"` 경로(build_report.py 272행)는 등급을 그대로 두고 상태만 바꾼다. 그래서 시트에는 `상태=해결` 행이 2개 있는데 콘솔과 validate 는 나란히 `해결 0` 을 찍는다. SKILL.md 6단계는 "해결로 뜬 항목을 채팅 요약에 반드시 포함" 하라고 하는데, 집계가 0 이면 그 지시를 따를 수 없다 | 두 곳 모두 `등급 == "해결" or 상태 == "해결"` 로 센다. validate 에 "상태=해결 행 수와 콘솔 집계가 같은가" 검사를 추가한다 |
| 27 | 중간 | `SKILL.md`(양쪽) | 226, 230-231 | `` \| `--force` \| 실행 이력이 뒤로 가도 강행. **쓰지 않는다** \| `` / `이게 걸리면 --force 로 뚫지 말고 **sync.py 로 최신본을 받은 폴더에서 다시 실행한다.**` | **안내된 해결책이 듣지 않는 경우가 있는데 그 경우가 문서에 없다.** 이미 sync 된 폴더인데도 가드가 걸릴 수 있다 — vendors.json 을 고쳐 `확인 필요` 가 정당하게 줄어든 경우다. 문서대로 다시 sync 해도 같은 메시지가 나오고, `--force` 는 금지돼 있어 진행할 길이 없다 | `--force` 금지는 유지하되 예외 절차를 명시한다: ① 원격 `state/last-run.json` 과 이번 결과를 대조해 ② 줄어든 거래처가 `거래 종료`·`단발·비정기` 등 **정당한 사유로 등급이 내려간 것인지** 확인하고 ③ 그 근거를 사용자에게 제시한 뒤 승인을 받아 `--force`. 가드 메시지 자체에도 이 경우를 한 줄 넣는다 |

### #26 실측 근거 (2026-09-06 4차)

원격 `state/last-run.json` 의 `확인 필요` 6곳 중 2곳이 이번 회차에 빠졌다.

| 거래처 | 직전 | 이번 | 시트의 `상태` | 콘솔·validate 집계 |
|---|---|---|---|---|
| 주식회사 디패스 | 확인 필요 | 거래 종료 (`until=2026-05`) | **해결** | 해결 0 |
| 서바이빙에프앤비 | 확인 필요 | 단발·비정기 (`cycle=비정기`) | **해결** | 해결 0 |

이 때문에 사용자에게 나간 1차 요약에 **"해결된 건은 없고"** 라는 사실과
다른 문장이 들어갔다. 사용자가 시트를 직접 열어 지적해서 정정했다.
**산출물은 옳았고 요약만 틀렸다** — validate 가 FAIL 0 을 낸 것도
같은 기준으로 세기 때문이라, 검증기로는 영영 안 걸린다.

### #27 실측 근거 (2026-09-06 4차)

세션 시작 때 codeload 로 받은 폴더에서 `push.py --dry-run` 이 중단됐다.

```
[중단] 실행 이력이 뒤로 갑니다: 같은 날짜(2026-09-06)인데
       확인 필요가 6건 → 4건 으로 줄어듭니다
       저장소에서 최신본을 받은 폴더에서 다시 실행하세요.
```

원격을 다시 받아 `diff -rq --exclude=state` 한 결과 `__pycache__` 외 차이
없음 — **이미 최신본이었다.** 감소분은 #26 표의 2곳이고 vendors.json
개선에 따른 정당한 하락이다. 문서가 지시한 조치(재 sync)로는 빠져나갈 수
없어 사용자 승인을 받아 `--force` 로 올렸다.
...
0. **#26 (`해결` 집계 누락) 을 가장 먼저 고친다.** 유일하게 사용자에게 나가는
   요약을 직접 틀리게 만든 결함이고, 실제로 한 번 틀린 요약이 전달됐다.
   validate 도 같은 기준으로 세므로 검증기로는 안 걸린다 — 코드 수정과
   검사 추가를 같은 회차에 한다
0-1. **#27 (`--force` 예외 절차 부재)** — 이번에 실제로 막혀서 사용자 승인으로
   뚫었다. 절차가 문서에 없으면 다음에도 같은 판단을 매번 즉석에서 하게 된다
