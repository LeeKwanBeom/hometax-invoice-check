...
**재확인 결과: 해결됨 11건 / 미해결 11건 / 근거없음 0건**
...
| 22 | 미수정 | **미해결** | sync.py | 34 | `# Claude 계정에 설치된 SKILL.md 는 사용자가 직접 재업로드해야 바뀐다.`<br>(SKILL.md 192행: `이 경로는 **쓰기 가능하다.** 사용자에게 재업로드를 시키지 말고 직접 반영한다.` — 이번에도 `touch` 로 쓰기 가능 실측) |
