...
> **6차 정기점검 (2026-09-07). 진단만 했고 코드·설정은 고치지 않았다.**
> 사용자가 채택할 항목을 고르기 전 상태다. 이번 회차에 저장소로 바뀐 것은
> `audit/last-audit.md`(이 파일) 와 `audit/checklist.md`(점검표 정본, 신규) 두 개뿐이다.
...
| 실 데이터 | **없음 — fixtures만 사용.** 업로드 6개는 `tests/fixtures/` 6개와 **바이트 동일**(md5 6/6 일치). 따라서 '실 데이터' 항목(당월 월 컬럼·기한 임박·신규 거래처·형식 변경)은 fixtures 기준 실측이지 실자료 검증이 아니다 |
...
1. **점검표 개정안 11건 중 사용자가 채택한 것이 `checklist.md` 에 반영되고 버전 줄이 갱신됐는가.** 특히 1번(`--as-of` 연도)은 채택 안 하면 다음 회차도 헛돈다.
