...
def t_resolved_status():
    ...
    with sandbox() as sb, tempfile.TemporaryDirectory() as out:
        ...
        check("새 행 경로에서 FAIL 없음", v.returncode == 0,
              v.stdout.strip()[-240:])
        # 1월(2026-01-05)에 until 4~5월은 아직 미래라 두 곳 다 거래 중이다.
        # 거래 종료 행이 있으면 안 되고, 해결은 새 행(등급열) 으로만 표기돼야 한다.
        # 6차 후속에서 "총 2건" 으로 바꿨던 것은 until 미래를 종료로 보던 회귀의
        # 증상이었다(until_passed 로 통일하며 되돌림).
